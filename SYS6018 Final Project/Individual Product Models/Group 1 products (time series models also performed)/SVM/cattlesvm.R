...
cattle2$name<-NULL
cattle2$date<-NULL
cattle2$ind_manager_spread<-as.factor(cattle2$ind_manager_spread)
cattle2$ind_other_report_spread<-NULL
cattle2$ind_swap_spread<-as.factor(cattle2$ind_swap_spread)
#==================ADF Tests=====================
...
test<-cattle2[seq(1:104),]
svm_cattle <- svm(weekly_change~., train)
summary(svm_cattle)
pred_cattle <- predict(svm_cattle, test)
plot(test$weekly_change,pch=16) #Plot the dataset
points(test$weekly_change, pred_cattle, col = "blue", pch=4)
act<-cattle[1:104,]$weekly_change
metrics(pred_cattle, act)