...
test<-oil2[seq(1:52),]
svm_oil <- svm(weekly_change~., train)
summary(svm_oil)
pred_oil <- predict(svm_oil, test)
plot(test$weekly_change,pch=16) #Plot the dataset
points(test$weekly_change, pred_oil, col = "blue", pch=4)
act<-oil[1:52,]$weekly_change
metrics(pred_oil, act)