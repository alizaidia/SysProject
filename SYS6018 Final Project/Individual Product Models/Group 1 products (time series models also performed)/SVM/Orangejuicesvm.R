...
svm_Orangejuice <- svm(weekly_change~., train)
summary(svm_Orangejuice)
pred_Orangejuice <- predict(svm_Orangejuice, test)
plot(test$weekly_change,pch=16) #Plot the dataset
points(test$weekly_change, pred_Orangejuice, col = "blue", pch=4)
act<-Orangejuice[1:104,]$weekly_change
metrics(pred_Orangejuice, act)