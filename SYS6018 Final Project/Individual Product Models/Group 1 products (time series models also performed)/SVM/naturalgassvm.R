...
svm_Naturalgas <- svm(weekly_change~., train)
summary(svm_Naturalgas)
pred_Naturalgas <- predict(svm_Naturalgas, test)
plot(test$weekly_change,pch=16) #Plot the dataset
points(test$weekly_change, pred_Naturalgas, col = "blue", pch=4)
act<-Naturalgas[1:104,]$weekly_change
metrics(pred_Naturalgas, act)