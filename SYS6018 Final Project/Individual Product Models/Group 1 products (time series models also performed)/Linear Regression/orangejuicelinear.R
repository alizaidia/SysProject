...
Orangejuicemodel<-lm(weekly_change~.,data=train)
summary(Orangejuicemodel)
#==================Model Adequacy===================
res<-resid(Orangejuicemodel)
qqnorm(res)
qqline(res)
#The normality assumption holds
vals<-fitted(Orangejuicemodel)
plot(vals,res)
#No pattern
#=========Predictions============================
preds <- vector(mode="numeric", length=104)
pred<-predict(Orangejuicemodel,test)
